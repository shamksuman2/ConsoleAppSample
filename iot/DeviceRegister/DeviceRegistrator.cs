...
namespace DeviceRegistration
{
    ...
    using Dover.Fleet.ServiceEngine.Library;
    using Flurl.Http;
        ...
        public async Task<bool> InitializeDeviceClient(IEnumerable<TimeSpan> retryIntervals)
        {
                ...
                && !string.IsNullOrWhiteSpace(_macAddress))
            {
                ...
                await retryPolicy.ExecuteAsync(async () =>
                {
                    ...
                    await _deviceClient.SendEventAsync(new Message().WithProperties(("message-type", "device-connected")), CancellationToken.None);
